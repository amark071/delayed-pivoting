...
/**
 @brief 采用混合主元的方法对未知格式的稠密矩阵进行 LU 分解，并采取延迟主元策略将待延迟的主元延迟到右下角。
  @param m 矩阵的行数。
  @param n 矩阵的列数。
  @param a 矩阵的指针。
 ...
 @return 
  ...
  @note 正常情况下每一步只在当前列做部分主元搜索；仅当当前列活动部分全零时，才在整个活动子矩阵中寻找最大绝对值元素，并同时执行行、列交换。
  @note 为获得更好性能我们在这里使用了 CBLAS 库来进行矩阵运算，确保在编译时链接相应的 BLAS 库。
  @note 参数非法时 void 函数立即返回，并设置 info=-1。 
 */
void delayed_getrf(int m, int n, double* a, int lda, int* ipiv, int* jpiv, int& info);
